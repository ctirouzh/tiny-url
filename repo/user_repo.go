...
func (r *UserRepository) CreateUser(createUserDto *dto.SignUp) (*model.User, error) {
	var user *model.User
	var count int
	...
	if err := r.session.
		Query(
			`INSERT INTO users (username, name, hashed_password, email, id, creation_date, last_login) VALUES (?, ?, ?, ?, ?, ?, ?)`,
			user.Username, user.Name, user.HashedPassword, user.Email, user.ID, user.CreationDate, user.LastLogin).
...
func (r *UserRepository) ValidateUser(credentials *dto.SignIn) (*model.User, error) {
	var user *model.User
